...
import asyncio
from Plugins.Func import connect_to_db, db_lock
...
async def send_periodic_message(client):
    while True:
        ...
        for user_id, rango, fecha_registro, dias in users_data:
            if dias is None or fecha_registro is None:
                continue
            ...
            try:
                if 0 < remaining_days <= 4:
                    await client.send_message(
                        user_id,
                        f"<b>⚠️ Attention, your subscription to Kurama Chk is about to expire, you have {remaining_days_int} days left. To renew your membership, contact @luisabinader1 | @AstraxOficial. ⚠️</b>"
                    )
            except Exception:
